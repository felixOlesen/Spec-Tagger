...
def test_find_all_spec_files():
    ...
    spec_crawler = SpecCrawler("features/", enabled_extensions={'.spec', '.feature', '.md', '.txt', '.allium'})
...
def test_find_all_test_files():
    ...
    test_crawler = TestCrawler("tests/", enabled_extensions={'.py', '.js', '.java', '.cpp', '.cs', '.rb', '.go', '.ts', '.php', '.swift', '.kt', '.m', '.scala', '.sh', '.pl', '.r', '.lua', '.hs', '.erl', '.ex', '.exs'})
...
def test_spec_crawler_extract_tags():
    ...
    assert len(spec_crawler.tag_data) > 0, "No tags extracted from the spec files."